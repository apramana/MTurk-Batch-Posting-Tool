...
* 
* Modified by Aaron Pramana <aaron@sociotopia.com> 2015
*/
...
class MechanicalTurk {
   ...
   private $MTURK_ROOT_URL = 'https://mechanicalturk.sandbox.amazonaws.com/?';
   ...
   public function setSandbox($mode){
      if($mode){
         $MTURK_ROOT_URL = 'https://mechanicalturk.sandbox.amazonaws.com/?';
      }
      else{
         $MTURK_ROOT_URL = 'https://mechanicalturk.amazonaws.com/?';
      }
   }
   ...
   function get_data($url) {
      ...
      $data = curl_exec($ch);
      if($data === false){
         die("Error: ".curl_error($ch)." Error #: ".curl_errno($ch));
      }
      curl_close($ch);
      return $data;
   }
   ...
   /*
    * Create a HIT from a template. Added by Aaron Pramana
    */
   public function createHitFromTemplate($title, $desc, $duration, $lifetime, $reward, $content){
      $ts = $this->Unix2UTC(time());
      //$url = $this->startUrl(); CHANGE THE BELOW . and &
      $url = 'Operation=CreateHIT';
      $url .= '&Signature=' . $this->generateSignature($this->MTURK_SERVICE, 'CreateHIT', $ts);
      $url .= '&Timestamp=' . $ts;
      $url .= '&Title=' . urlencode($title);
      $url .= '&Description=' . urlencode($desc);
      $url .= '&AssignmentDurationInSeconds=' . $duration;
      $url .= '&LifetimeInSeconds=' . $lifetime;
      $url .= '&Reward.1.Amount=' . $reward;
      $url .= '&Reward.1.CurrencyCode=USD';
      $url .= '&Question=' . urlencode($content);
      $url .= '&QualificationRequirement.1.QualificationTypeId=000000000000000000L0';
      $url .= '&QualificationRequirement.1.Comparator=GreaterThanOrEqualTo';
      $url .= '&QualificationRequirement.1.IntegerValue=90';
      ...
      $response = file_get_contents($this->startUrl(), false, $context);
      return $response;
   }
